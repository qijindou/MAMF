...
class Indikeppar:
    def __init__(self, filename, filename_comparison):
    ...
    def loadDailyReturn(self):
        for idx, item in self.data.iterrows():
            try:
                self.data.loc[idx, 'R'] = self.data.loc[idx, 'value'] - self.data.loc[idx-1, 'value']
            except:
                pass

        for idx, item in self.data_comparison.iterrows():
            try:
                self.data_comparison.loc[idx, 'R'] = self.data_comparison.loc[idx, 'value'] \
                                                     - self.data_comparison.loc[idx-1, 'value']
            except:
                pass
...
class Indicator:
    ...
    def add_indicators(self, ma=[20, 50, 100]):
        for idx, item in self.data.iterrows():
            for m in ma:
                self.data[str(m) + 'MA'] = self.data['value'].rolling(window=m).mean()
    ...
    def rsi(self, window=14, trend_ma=5, show=False):
        ...
        for idx, item in self.data.iterrows():
            try:
                self.data.loc[idx, 'Gain'] = self.data.loc[idx, 'value'] - self.data.loc[idx - 1, 'value'] if \
                self.data.loc[idx, 'value'] - self.data.loc[idx - 1, 'value'] > 0 else 0
                self.data.loc[idx, 'Loss'] = self.data.loc[idx, 'value'] - self.data.loc[idx - 1, 'value'] if \
                self.data.loc[idx, 'value'] - self.data.loc[idx - 1, 'value'] < 0 else 0
            except:
